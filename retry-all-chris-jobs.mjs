...
/**
 * Retry all 10 Chris Hemsworth outline generation jobs (fast mode)
 * Usage: SUPABASE_SERVICE_ROLE_KEY=your_key node retry-all-chris-jobs.mjs
 */
...
if (!SUPABASE_SERVICE_KEY) {
  ...
  console.error('Usage: SUPABASE_SERVICE_ROLE_KEY=your_key node retry-all-chris-jobs.mjs');
  process.exit(1);
}
...
const jobsToRetry = [
  ...
  {
    id: "a685484f-088c-4f34-bc6f-de716334128f",
    post_title: "Complete Chris Hemsworth Diet Plan for Lean Muscle and Energy",
    post_keyword: "chris hemsworth diet plan"
  },
  {
...
async function retryJobs() {
  ...
  console.log('   SELECT id, post_keyword, status, fast_mode, heartbeat, updated_at');
