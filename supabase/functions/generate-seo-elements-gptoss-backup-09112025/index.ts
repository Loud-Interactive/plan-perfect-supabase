...
import { generateOrGetCachedSchema, buildEnhancedPrompt, storeCustomSeoData } from '../utils/custom-fields.ts';
...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch pairs data for the domain
    let pairsData: Record<string, any> = {};
    if (domain) {
      // Clean domain for lookup (remove www. if present)
      ...
      } else if (pairsRawData && pairsRawData.length > 0) {
        // Convert to object
        pairsRawData.forEach(pair => {
          pairsData[pair.key] = pair.value;
        });
        console.log(`Loaded ${Object.keys(pairsData).length} pairs for domain: ${cleanDomain}`);
      } else {
    ...
    function buildBrandSection(pairsData: Record<string, any>): string {
      ...
      if (pairsData.special_ai_instructions) {
        brandSection += `\n\nHere's the ${brandName} special AI instructions:\n${pairsData.special_ai_instructions}\n\n You must follow these instructions strictly.`;
      }
    ...
    if (customFieldsDescription) {
      console.log('Generating custom schema for fields:', customFieldsDescription);
      try {
        customSchema = await generateOrGetCachedSchema(customFieldsDescription);
        console.log('Custom schema generated:', JSON.stringify(customSchema, null, 2));
      } catch (error) {
        console.error('Error generating custom schema:', error);
        // Continue without custom fields
      }
    }

    // Full prompt with all details
    const basePrompt = `You are tasked with creating SEO-optimized elements for a webpage based on its URL, content, and keyword data. Your goal is to improve the page's search engine visibility and user engagement.
...
If the keywords listed seem to be placeholders like ("keyword 1" or "keyword 2" or "keyword not found" or "null") then select your own keywords based on the content of the page, the URL of the page and other information you know about the brand/company.
${brandInfoSection}
...
You must first make your selections from the keyword data (unless the keyword data appears to be placeholder information). Select a primary, secondary, and tertiary keyword. Keep in mind that the keywords are already provided to you in preferential order, but you have the freedom to select any of the keywords if you feel the order provided wouldn't make sense because the terms are too similar (plurals), don't truly reflect the page content's intent, are off-brand, are a form of a branded term or are somehow a competitor's brand name. That said, unless the keywords appear to be placeholder keywords, then you MUST select from the LIST given. If there are no keywords given at all then you must select your own keywords the best you can and proceed. ONLY do this IF there are no keywords provided. Then you must create the SEO elements based on the following instructions. IMPORTANT: Never select a branded term as a keyword. Never select a keyword outside the list provided (unless the list provided are placeholder terms or there was no list provided).

You must be strict with the character limits for the title (70 characters), meta description (155 characters), H1 heading (120 characters), H2 heading (120 characters), H4 heading (120 characters), Paragraph (60,000 characters) - do not exceed the character limitations.

Based on your analysis, create the following elements:

1. Title: Create an SEO-optimized title that is concise (70 characters) and accurately represents the page content. You MUST include at least one of the top 5 keywords from the provided keyword list or the keywords you selected because no keywords were provided or because you felt the keywords provided were placeholders. Choose the most relevant keywords, prioritizing those at the top of the list (higher importance) but with flexibility to select others if necessary to meet other requirements. ${pairsData.brand_name || pairsData.business_info ? `Always end the title with " | ${pairsData.business_info || pairsData.brand_name}" unless that drives you over the 70-character limitation, then you should end the title with " | ${pairsData.also_known_as || pairsData.brand_short || pairsData.brand_name}".` : ''} You must stay within the character limitation of 70 characters${pairsData.brand_name ? ', including one of the above options as a title suffix' : ''}.

2. Meta Description: Write a compelling meta description (155 characters) that summarizes the page content and entices a click when seen in search results. You MUST include at least two primary keywords from the provided keyword list or the keywords you selected because no keywords were provided or because you felt the keywords provided were placeholders. The description should entice users to click through to the page by highlighting a benefit or addressing a pain point that is solved by the content of the page. ${pairsData.usp ? 'Also consider the USP listed above and the other branding information from above when creating a compelling meta description.' : ''}

3. H1 Heading: Craft a primary heading that clearly states the main topic of the page. You MUST include at least one of the top-provided keywords from the keyword list (or the keywords you selected because no keywords were provided or because you felt the keywords provided were placeholders) -- preferably the most important keyword should be selected. The H1 should be similar to the title but can be longer and more descriptive. Relevance to the page's content is weighted the heaviest here, but requirements still state to stay within the options provided in the keyword list. If you feel there is a complete disconnect between the keywords provided or selected by you for the previously mentioned reasons, then ignore the keywords completely and create a proper H1 heading that works with the page's content. It's vital that you only utilize this option in extreme cases. DO NOT CREATE AN H1 HEADING THAT INCLUDES CONTENT THAT MAY BECOME DATED (SUCH AS TEMPORARY SALES OR PROMOTIONS). EXAMPLE DO NOT DO THE FOLLOWING:
...
4. H2 Heading: Create a secondary heading that acts as a page-level subheading. This should be broader than the H1 and set up the category or section-level context. To expand keyword coverage, you MUST use different keywords from those used in the H1. Choose from the keyword list or select from those you had to select because no keywords were provided or because they were placeholder keywords, focusing on those that complement the primary keywords.
...
5. H4 Heading: Create a sectional heading directly related to the paragraph's content, which will be placed directly below this H4 heading on the page. This heading should be more specific and focused on the paragraph topic or paragraph text. It should be more specific than the H2 and directly introduce the paragraph's content.

6. Paragraph: Write five paragraphs (with three sentences each) that provide valuable information related to the H4 heading above it. These paragraphs should naturally integrate the primary keyword, secondary keyword and at least two additional keywords from the provided list. If no keywords are supplied or the keywords appear to be placeholders, assume keywords and use them accordingly. Ensure that whichever keywords are used fit contextually and provide meaningful value to the reader. Do not keyword stuff. The content should be about 2250 characters in length.
...
Your final output should contain only the SEO elements within the specified tags. Do not include any explanations, additional text, or the keyword data outside of these tags.`;
    ...
    // Build enhanced prompt if custom schema exists
    if (customSchema) {
      enhancedPrompt = buildEnhancedPrompt(basePrompt, customSchema, pageContent, pageKeywords);
    } else {
    ...
    const metadata = {
      ...
      hasCustomFields: customSchema !== null,
    ...
    // Store custom SEO data if we have custom schema
    if (customSchema && pageId) {
      try {
        await storeCustomSeoData(supabaseClient, pageId, seoData, customSchema, customFieldsDescription);
      } catch (error) {
        console.error('Error storing custom SEO data:', error);
...
<paragraph>Your paragraph here</paragraph>
        ...
        if (existingRecords && existingRecords.length > 0) {
          ...
          if (customSchema) {
            updateData.custom_seo_data = seoData.customFields || null;
            updateData.custom_schema = customSchema;
            updateData.custom_fields_hash = customFieldsDescription ? 
              await crypto.subtle.digest('SHA-256', new TextEncoder().encode(customFieldsDescription))
          ...
          // Add custom fields if we have them
          if (customSchema) {
            insertData.custom_seo_data = seoData.customFields || null;
            insertData.custom_schema = customSchema;
            insertData.custom_fields_hash = customFieldsDescription ? 
              await crypto.subtle.digest('SHA-256', new TextEncoder().encode(customFieldsDescription))
                .then(buffer => Array.from(new Uint8Array(buffer)).map(b => b.toString(16).padStart(2, '0')).join('')) 
              : null;
          }
    ...
    return new Response(
      JSON.stringify({
        ...
        customFieldsGenerated: customSchema !== null
      }),
      {
        headers: {
          ...corsHeaders,
