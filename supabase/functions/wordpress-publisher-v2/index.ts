// Content Perfect WordPress Publishing Service V2 - Supports Updates for Regenerated Content
...
serve(async (req) => {
  ...
  try {
    ...
    if (path === "/wordpress-publisher-v2" && req.method === "POST") {
      return await publishContent(req);
    } else if (path === "/wordpress-publisher-v2/status" && req.method === "GET") {
      return await getStatus(req);
    } else if (path === "/wordpress-publisher-v2/retry" && req.method === "POST") {
...
async function publishContent(req: Request) {
  try {
    // Parse request body
    const { taskId, forceUpdate = false } = await req.json();
    ...
    const isUpdate = !!task.live_post_url;
    const operation = isUpdate && !forceUpdate ? "update" : "create";

    console.log(`Operation: ${operation} for task ${taskId} (has URL: ${isUpdate}, force: ${forceUpdate})`);

    // Check if this is a regeneration (content changed since last publish)
    let isRegenerated = false;
    if (isUpdate) {
      // Check if content has changed since last publish
      const lastPublished = task.last_published_at;
      const contentUpdated = task.updated_at;
      
      if (lastPublished && contentUpdated) {
        const lastPubDate = new Date(lastPublished);
        const updateDate = new Date(contentUpdated);
        isRegenerated = updateDate > lastPubDate;
      }
      
      console.log(`Content regenerated: ${isRegenerated} (last published: ${lastPublished}, updated: ${contentUpdated})`);
    }

    // If content hasn't changed and we're not forcing update, skip
    if (isUpdate && !isRegenerated && !forceUpdate) {
      return new Response(
        JSON.stringify({ 
          message: "Content unchanged since last publish", 
          url: task.live_post_url,
          last_published: task.last_published_at
        }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    // Publish content to WordPress
    const result = await publishToWordPress(task, operation);
...
async function getStatus(req: Request) {
  try {
    ...
    // Get regenerated tasks count (content updated after last publish)
    const { data: regeneratedTasks, error: regenError } = await supabaseClient
      .from("tasks")
      .select("task_id, updated_at, last_published_at")
      .eq("status", "Complete")
      .not("live_post_url", "is", null);

    if (regenError) throw regenError;

    // Count how many are regenerated
    const regeneratedCount = regeneratedTasks?.filter(task => {
      if (!task.last_published_at) return false;
      const lastPub = new Date(task.last_published_at);
      const updated = new Date(task.updated_at);
      return updated > lastPub;
    }).length || 0;
    ...
    return new Response(
      JSON.stringify({
        ...
        regeneratedTasksCount: regeneratedCount,
  ...
  try {
    ...
    for (const retryTask of retryTasks) {
      try {
        ...
        const operation = task.live_post_url ? "update" : "create";
 ...
 * Publish content to WordPress
 */
async function publishToWordPress(task: any, operation: "create" | "update") {
  try {
    ...
    // Check if monthly post limit has been reached (only for new posts)
    if (operation === "create" && monthlyPostLimit > 0 && monthlyPostCount >= monthlyPostLimit) {
    ...
    const payload = {
      ...
      operation: operation,
      wordpress_post_id: task.wordpress_post_id // Include if we're updating
    };
    ...
    const response = await axiod.post(endpoint, payload, {
      headers: {
        ...
        "X-Operation": operation // Tell plugin if this is create or update
      },
    ...
    if (response.data.success) {
      // Update task with live post URL and WordPress post ID
      ...
      if (response.data.post_id) {
        updateData.wordpress_post_id = response.data.post_id;
      }
      ...
      // Increment the monthly post count for new posts only
      if (operation === "create") {
        ...
        await logPublication(task.task_id, "updated", {
          ...response.data,
          operation: operation
        });
      }
      ...
      return {
        ...
        operation: operation,
        wordpress_post_id: response.data.post_id
      };
    } else {
