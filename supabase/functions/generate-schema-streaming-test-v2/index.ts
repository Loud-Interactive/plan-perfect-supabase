// supabase/functions/generate-schema-streaming-test-v2/index.ts
...
4. **EXTENDED PROPERTIES YOU CAN EXTRAPOLATE (BUT ARE STILL OPTIONAL):**
   
   These properties CAN be included if you find supporting evidence, but you are NOT required to include them:
   
   📊 **Publisher/Organization Details** (can be extrapolated from):
   - Domain name (e.g., example.com → "Example")
   - Footer information
   - About sections
   - Copyright notices
   - Logo images
   
   👤 **Author Information** (look for):
   - Bylines ("By John Smith")
   - Author boxes
   - Bio sections
   - Staff/team pages references
   
   🌐 **Social Media & Contact** (may appear in):
   - Social media icons/links
   - Footer contact sections
   - Author profiles
   - Share buttons with profiles
   
   🔗 **Related Content** (can be found in):
   - "Related Articles" sections
   - "You might also like" sections
   - Sidebar recommendations
   - Category/tag listings
   
   📍 **Location Information** (extractable from):
   - Address mentions
   - Maps/location widgets
   - Event venues
   - Store locations
   
   💬 **Interaction Data** (if visible):
   - Comment counts
   - Like/share counts
   - View counts
   - Rating displays
   
   🎥 **Media Content** (look for):
   - Embedded videos (YouTube, Vimeo, etc.)
   - Audio players
   - Image galleries
   - Infographics
   
   🛒 **Product/Offer Information** (for e-commerce):
   - Product details
   - Pricing information
   - Availability status
   - Buy buttons
   ...
   REMEMBER: Just because these CAN be extrapolated doesn't mean they SHOULD be included. Only include them if you find actual evidence in the content.

5. **DOMAIN PREFERENCES DATA:**
...
8. **REASONING APPROACH:**
   ...
   1. First, check if there's direct evidence in the content
   2. If not directly stated, check if it can be reasonably extrapolated (see section 4)
   3. If uncertain, DO NOT include the field
   4. Document your reasoning internally but don't include it in the output

9. **VALIDATION CHECKLIST FOR EACH FIELD:**
   Before including ANY field, ask yourself:
   - Can I point to specific text/elements in the content that supports this value?
   - Am I extracting this from the actual content, not inventing it?
   - Is this field relevant to the type of content I'm analyzing?
   - If extrapolated, is my inference reasonable and defensible?
...
**FINAL REMINDER:**
The list above shows POSSIBLE fields you could include, but you should ONLY include fields where you have actual data from the content or can reasonably extrapolate it. DO NOT feel obligated to include all fields listed above.

**OUTPUT INSTRUCTIONS:**
Return ONLY the JSON-LD schema code. No explanation, no wrapper text, just the complete JSON-LD schema markup.

Remember: Every single field in your output (except required fields like @context and @type) must be traceable back to specific content in the article, domain preferences data, or reasonable extrapolation. If you cannot find evidence for a field, DO NOT include it.
`
          ...
          const chatCompletion = await groq.chat.completions.create({
            messages: [
              {
                role: "system",
                content: "You are an expert SEO specialist focused on creating JSON-LD schema markup. Use careful reasoning to identify what information can be reliably extracted or inferred from the content, but always err on the side of accuracy over completeness."
              },
              {
                role: "user",
                content: prompt
