...
serve(async (req)=>{
  ...
  try {
    ...
    const fast = requestData.fast !== false; // Default to true (fast mode) unless explicitly set to false
    if (!job_id) {
      return new Response(JSON.stringify({
        error: 'content_plan_outline_guid or job_id is required'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
    ...
    if (fast) {
      ...
      // Pass the correct parameter name based on whether it's a UUID or integer
      const fastPayload = isUUID 
      ...
      if (!fastResponse.ok) {
        ...
        await supabase.from('content_plan_outlines').update({
      ...
      // Continue with slow mode below
        ...
        await supabase.from('content_plan_outlines').update({
          status: 'Fast outline regeneration started',
          updated_at: new Date().toISOString()
        }).eq('guid', job_id);
    ...
    const { data: job, error: jobError } = await supabase.from('outline_generation_jobs').select('*').eq('id', job_id).single();
    if (jobError || !job) {
      return new Response(JSON.stringify({
    ...
    const { data: originalOutlineData, error: originalOutlineError } = await supabase.from('content_plan_outlines_ai').select('*').eq('job_id', job_id).order('created_at', {
      ascending: false
    }).limit(1).single();
    if (originalOutlineError || !originalOutlineData) {
      return new Response(JSON.stringify({
        error: `Original outline not found: ${originalOutlineError?.message || 'Unknown error'}`
      }), {
        status: 404,
        headers: {
          ...corsHeaders,
    ...
    // Get search results
    const { data: searchResults, error: searchResultsError } = await supabase.from('outline_search_results').select('*').eq('job_id', job_id);
    if (searchResultsError) {
      console.error(`Error fetching search results: ${searchResultsError.message}`);
    }
    // Get the analyses of search results
    const { data: urlAnalyses, error: urlAnalysesError } = await supabase.from('outline_url_analyses').select('*').eq('job_id', job_id);
    if (urlAnalysesError) {
    ...
    (async ()=>{
      try {
        ...
        await supabase.from('outline_generation_jobs').update({
          status: 'regenerating',
          updated_at: new Date().toISOString()
        }).eq('id', job_id);
        // Add status update
        ...
        const categoryInfo = searchResults ? Array.from(new Set(searchResults.map((c)=>c.search_category))).map((category)=>{
        ...
        const categoryContext = categoryInfo.length > 0 ? `I searched for information using different types of search terms:
      ...
      "subheadings": ["Summary", "Final thoughts", "Call to action"]
        ...
        try {
          ...
          if (outlineJson.sections && outlineJson.sections[0]) {
            outlineJson.sections[0].subheadings = [
              ...outlineJson.sections[0].subheadings || [],
              "Regenerated version with improvements"
            ];
          }
        }
        // Add status update
        ...
        await supabase.from('content_plan_outlines_ai').insert({
          job_id,
          outline: outlineJson
        });
