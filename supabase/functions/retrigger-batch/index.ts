// Retrigger workflow for a specific batch of crawl jobs
...
serve(async (req) => {
  ...
  try {
    ...
    // Get batch ID from URL or request body
    ...
    const batchIdToProcess = batchId || requestBody.batchId;
    
    if (!batchIdToProcess) {
      return new Response(
        JSON.stringify({
          success: false,
          error: 'Batch ID is required. Provide it as a query parameter (?batchId=...) or in request body {"batchId": "..."}'
        }),
        {
    ...
    console.log(`Processing batch ID: ${batchIdToProcess}`);
    
    const supabase = createClient(SUPABASE_URL, SUPABASE_KEY);
    
    // First ensure HTML is copied from crawl_jobs to pages
    await supabase.rpc('exec_sql', {
      query: `
        ...
        WHERE 
          ...
          AND cj.batch_id = '${batchIdToProcess}'
    ...
    const { data: jobs } = await supabase.rpc('exec_sql', {
      query: `
        ...
        WHERE 
          j.status = 'completed'
          AND j.batch_id = '${batchIdToProcess}'
        ORDER BY 
          j.completed_at DESC
      `
    });
    ...
    if (!jobs || jobs.length === 0) {
      return new Response(
        JSON.stringify({
          success: true,
          message: `No completed jobs found for batch ${batchIdToProcess}`,
          count: 0
        }),
        {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    ...
    console.log(`Found ${jobs.length} completed jobs for batch ${batchIdToProcess}`);
    ...
    console.log(`Batch ${batchIdToProcess} processing complete`);
    
    return new Response(
      JSON.stringify({
        success: true,
        message: `Processed ${jobs.length} jobs for batch ${batchIdToProcess}`,
