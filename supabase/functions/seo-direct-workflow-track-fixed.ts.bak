// Simplified SEO Direct Workflow with direct tracking - FIXED VERSION
  ...
  try {
        ...
        if (keywordResponse.ok) {
          try {
            ...
            if (keywordResult.success) {
              ...
              gscSuccessful = true; // Mark as successful since we have alternative keywords
            } else {
    ...
    try {
      ...
      if (!gscResponse.ok) {
        ...
        await extractKeywordsFromContent();
      } else {
        gscSuccessful = true;
        ...
        try {
          ...
          console.error(`Error checking keyword count: ${countError.message}`);
        }
      }
    } catch (error) {
    ...
    try {
      ...
      try {
        // Call generate-seo-elements function
        const elementsResponse = await fetch(`${SUPABASE_URL}/functions/v1/generate-seo-elements`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${SUPABASE_KEY}`
    ...
    // Step 6: Update tracking record if we have one
    if (trackingId) {
