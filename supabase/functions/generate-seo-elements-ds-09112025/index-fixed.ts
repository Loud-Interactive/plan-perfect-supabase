// PagePerfect: generate-seo-elements-ds (DeepSeek Reasoner version) - FIXED
...
<paragraph>Your paragraph here</paragraph>
    ...
    let pairsData: Record<string, string> = {};
    if (domain) {
      ...
      if (pairsError) {
        ...
        console.log(`Loaded ${Object.keys(pairsData).length} pairs for domain: ${domain}`);
      } else {
        console.log(`No pairs data found for domain: ${domain}`);
      }
    } else {
    ...
    function buildBrandSection(pairsData: Record<string, string>): string {
      let brandSection = '';
      ...
      if (pairsData.competitor_names) {
        ...
        brandSection += `\n\nNEVER mention the above competitors' brand names in your content.`;
      }
      
      if (pairsData.competitor_domains) {
        brandSection += `\n\nHere's the ${brandName} competitors domains:\n${pairsData.competitor_domains}`;
        brandSection += `\n\nNEVER mention the above competitors' domains in your content.`;
      }
    ...
    too similar (plurals), don't truly page the page content's intent, are off-brand, are a form of a branded term or are somehow a competitor's brand name. That said, unless the keywords appear to 
    ...
    top of the list (higher importance) but with flexibility to select others if necessary to meet other requirements. ${pairsData.brand_name || pairsData.business_info ? `Always end the title with " | ${pairsData.business_info || 
    pairsData.brand_name}" unless that drives you over the 70-character limitation, then you should end the title with " | ${pairsData.also_known_as || pairsData.brand_short || 
    pairsData.brand_name}".` : ''} You must stay within the character limitation of 70 characters${pairsData.brand_name ? ', including one of the above options as a title suffix' : ''}.
    ...
    // Use the full prompt with conditional brand information
