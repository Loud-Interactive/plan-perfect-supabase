// PagePerfect: generate-seo-elements-ds (DeepSeek R1 version with custom fields)
// Function to generate SEO-optimized elements using DeepSeek with reasoning and custom field support
...
const domainRegexCache = new Map();
...
async function loadDomainRegex(supabaseClient, domain) {
  ...
  try {
    ...
    const { data, error } = await supabaseClient.from('pairs').select('value, last_updated').eq('domain', normalizedDomain).eq('key', 'branded_terms_regex_code').order('last_updated', {
      ascending: false
    }).limit(1);
    if (error) {
...
async function isValidKeyword(keyword, pattern) {
...
serve(async (req)=>{
  ...
  try {
    ...
    // Parse request body with custom field support
    const { 
      url, 
      pageId, 
      deepseekApiKey, 
    ...
    console.log(`Generating SEO elements for ${url || `pageId: ${pageId}`} using DeepSeek R1`);
    // Get page data
    let pageData;
    let pageContent;
    let pageKeywords;
    ...
    if (pageId) {
      ...
      const { data, error } = await supabaseClient.from('pages').select('url, html, domain, http_status, canonical_url, original_url').eq('id', pageId).single();
      if (error || !data) {
      ...
      try {
        const { data: recKeywords, error: recError } = await supabaseClient.from('page_seo_recommendations').select('keywords').eq('page_id', pageId).single();
      ...
      if (!pageKeywords || pageKeywords.length === 0) {
        const { data: keywordsData, error: keywordsError } = await supabaseClient.from('gsc_keywords').select('keyword, clicks, impressions, position, ctr').eq('page_id', pageId).order('impressions', {
          ascending: false
        }).limit(20);
    ...
    if (pageKeywords && pageKeywords.length > 0) {
      ...
      keywordsText += finalKeywords.map((kw, index)=>{
        ...
        const stats = kw.impressions ? `clicks: ${kw.clicks}, impressions: ${kw.impressions}, position: ${typeof kw.position === 'number' ? kw.position.toFixed(1) : kw.position}` : kw.relevance ? `relevance: ${kw.relevance}/10, search intent: ${kw.search_intent || 'unknown'}` : 'no metrics available';
    ...
    if (domain) {
      ...
      const { data: pairsRawData, error: pairsError } = await supabaseClient.from('pairs').select('key, value').eq('domain', cleanDomain);
      if (pairsError) {
    ...
    function buildBrandSection(pairsData) {
      let brandSection = '';
    ...
    const fullPrompt = `You are tasked with creating SEO-optimized elements for a webpage based on its URL, content, and keyword data. Your goal is to improve the page's search engine visibility 
    and user engagement.
    ...
    const prompt = customSchema 
      ? buildEnhancedPrompt(fullPrompt, customSchema, pageContent, pageKeywords || [])
      : fullPrompt;
    // Call DeepSeek R1 API with logging
    console.log(`Calling Groq API with logging for ${url || `pageId: ${pageId}`}`);
    ...
    if (pageId) {
      ...
      try {
        ...
        const { data: pageData, error: pageError } = await supabaseClient.from('pages').select('url').eq('id', pageId).single();
        if (pageError) {
        ...
        if (!hasValidElements) {
          ...
          const { response: retryModelResponse, thinking: retryReasoning } = await callDeepSeekWithLogging(FUNCTION_NAME + '_retry1', retryPrompt, apiKey, domain, {
            ...metadata,
            retry: 1
          });
          const firstRetry = {
            content: retryModelResponse,
            thinking: retryReasoning
          };
...
<paragraph>Your paragraph here</paragraph>
            ...
            const { response: retry2ModelResponse, thinking: retry2Reasoning } = await callDeepSeekWithLogging(FUNCTION_NAME + '_retry2', simplifiedPrompt, apiKey, domain, {
              ...metadata,
              retry: 2
            });
            const secondRetry = {
              content: retry2ModelResponse,
              thinking: retry2Reasoning
            };
            ...
            if (!retryData.title || !retryData.h1 || !retryData.h4) {
              ...
              const { response: finalRetryModelResponse, thinking: finalRetryReasoning } = await callDeepSeekWithLogging(FUNCTION_NAME + '_retry_final', simplifiedPrompt, apiKey, domain, {
                ...metadata,
                retry: 3,
                final_attempt: true
              });
          ...
          if (hasValidElementsAfterRetry) {
            console.log("Successfully generated elements after Groq retries");
          } else {
            console.error("Failed to generate valid elements even after multiple Groq retries");
        ...
        const { data: existingRecords, error: checkError } = await supabaseClient.from('page_seo_recommendations').select('id').eq('page_id', pageId);
        if (checkError) {
        ...
        if (existingRecords && existingRecords.length > 0) {
          ...
          const { error: updateError } = await supabaseClient.from('page_seo_recommendations').update(updateData).eq('id', recordId);
          if (updateError) {
          ...
          const { error: insertError } = await supabaseClient.from('page_seo_recommendations').insert(insertData);
          if (insertError) {
            ...
            if (insertError.message.includes('violates unique constraint')) {
              ...
              const { error: deleteError } = await supabaseClient.from('page_seo_recommendations').delete().eq('page_id', pageId);
              if (deleteError) {
                console.error(`Delete error: ${deleteError.message}`);
              } else {
                // Try insert again
                const { error: reinsertError } = await supabaseClient.from('page_seo_recommendations').insert({
                  page_id: pageId,
                  url: pageUrl,
                  title,
                  meta_description: metaDescription,
...
function htmlToMarkdown(html) {
  ...
  let markdown = cleanText(html)// Remove scripts and styles
  .replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '').replace(/<style\b[^<]*(?:(?!<\/style>)<[^<]*)*<\/style>/gi, '')// Convert headings
  .replace(/<h1[^>]*>([\s\S]*?)<\/h1>/gi, '# $1\n\n').replace(/<h2[^>]*>([\s\S]*?)<\/h2>/gi, '## $1\n\n').replace(/<h3[^>]*>([\s\S]*?)<\/h3>/gi, '### $1\n\n').replace(/<h4[^>]*>([\s\S]*?)<\/h4>/gi, '#### $1\n\n').replace(/<h5[^>]*>([\s\S]*?)<\/h5>/gi, '##### $1\n\n').replace(/<h6[^>]*>([\s\S]*?)<\/h6>/gi, '###### $1\n\n')// Convert paragraphs and line breaks
  .replace(/<p[^>]*>([\s\S]*?)<\/p>/gi, '$1\n\n').replace(/<br\s*\/?>/gi, '\n')// Convert lists
  .replace(/<ul[^>]*>([\s\S]*?)<\/ul>/gi, '$1\n').replace(/<ol[^>]*>([\s\S]*?)<\/ol>/gi, '$1\n').replace(/<li[^>]*>([\s\S]*?)<\/li>/gi, '- $1\n')// Convert links
  .replace(/<a[^>]*href="([^"]*)"[^>]*>([\s\S]*?)<\/a>/gi, '[$2]($1)')// Convert images
  .replace(/<img[^>]*src="([^"]*)"[^>]*alt="([^"]*)"[^>]*>/gi, '![$2]($1)').replace(/<img[^>]*alt="([^"]*)"[^>]*src="([^"]*)"[^>]*>/gi, '![$1]($2)').replace(/<img[^>]*src="([^"]*)"[^>]*>/gi, '![]($1)')// Convert strong/bold and em/italic text
  .replace(/<strong[^>]*>([\s\S]*?)<\/strong>/gi, '**$1**').replace(/<b[^>]*>([\s\S]*?)<\/b>/gi, '**$1**').replace(/<em[^>]*>([\s\S]*?)<\/em>/gi, '*$1*').replace(/<i[^>]*>([\s\S]*?)<\/i>/gi, '*$1*')// Remove all other HTML tags
  .replace(/<[^>]*>/g, '')// Fix multiple consecutive line breaks
  .replace(/\n\s*\n\s*\n/g, '\n\n');
