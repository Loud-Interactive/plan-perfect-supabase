// Combined version of SEO Direct Workflow with DeepSeek → GPT-OSS fallback
// First tries DeepSeek, falls back to Groq GPT-OSS-120B on error
...
serve(async (req) => {
  ...
  try {
    ...
    let modelUsed = null; // Track which model was used for SEO generation
    ...
    try {
      ...
      // Try DeepSeek first
      console.log(`Debug: Attempting SEO generation with DeepSeek model...`);
      try {
        ...
        if (elementsResponse) {
          ...
          console.log(`Debug: DeepSeek response OK for ${page.url}`);
          
          try {
            const elementsResult = JSON.parse(elementsResponseText);
            
            if (elementsResult.success && elementsResult.seoElements) {
              console.log(`Debug: DeepSeek generated SEO elements: Title=${elementsResult.seoElements.title?.substring(0, 30)}..., H1=${elementsResult.seoElements.h1?.substring(0, 30)}...`);
              success = true;
              seoSuccess = true;
              modelUsed = 'deepseek';
            } else {
          ...
          console.error(`Debug: Failed to connect to DeepSeek service after all retries`);
          errorMessage = 'Could not connect to DeepSeek SEO generation service';
        }
      } catch (apiError) {
        console.error(`Debug: DeepSeek API call exception: ${apiError.message}`);
        errorMessage = `DeepSeek SEO generation error: ${apiError.message}`;
      }
      
      // If DeepSeek failed, try GPT-OSS as fallback
      if (!success) {
        console.log(`Debug: DeepSeek failed, attempting fallback to GPT-OSS-120B...`);
        ...
        try {
          ...
          if (elementsResponse) {
            let gptossResponseText = '';
            try {
              gptossResponseText = await elementsResponse.text();
              console.log(`Debug: GPT-OSS response status: ${elementsResponse.status}`);
              console.log(`Debug: GPT-OSS response preview: ${gptossResponseText.substring(0, 200)}...`);
            } catch (textError) {
              console.error(`Debug: Error getting GPT-OSS response text: ${textError.message}`);
            }
            
            if (elementsResponse.ok) {
              console.log(`Debug: GPT-OSS response OK for ${page.url}`);
              
              try {
                const gptossResult = JSON.parse(gptossResponseText);
                
                if (gptossResult.success && gptossResult.seoElements) {
                  console.log(`Debug: GPT-OSS generated SEO elements: Title=${gptossResult.seoElements.title?.substring(0, 30)}..., H1=${gptossResult.seoElements.h1?.substring(0, 30)}...`);
                  success = true;
                  seoSuccess = true;
                  modelUsed = 'gpt-oss';
                  errorMessage = null; // Clear any previous error since GPT-OSS succeeded
                } else {
                  console.error(`Debug: GPT-OSS returned success=false or missing data: ${JSON.stringify(gptossResult)}`);
                }
              } catch (parseError) {
                console.error(`Debug: Error parsing GPT-OSS response: ${parseError.message}`);
              }
            } else {
              console.error(`Debug: GPT-OSS generation failed: ${elementsResponse.status} - ${gptossResponseText}`);
              if (!errorMessage) {
                errorMessage = `Both DeepSeek and GPT-OSS failed (GPT-OSS: ${elementsResponse.status})`;
              }
            }
          } else {
            console.error(`Debug: Failed to connect to GPT-OSS service after all retries`);
            if (!errorMessage) {
              errorMessage = 'Could not connect to either DeepSeek or GPT-OSS services';
            }
          }
        } catch (gptossError) {
          console.error(`Debug: GPT-OSS API call exception: ${gptossError.message}`);
          if (!errorMessage) {
            errorMessage = `Both models failed: ${gptossError.message}`;
          }
        }
      }
      ...
      if (!success) {
        console.log(`Debug: Both DeepSeek and GPT-OSS failed, using fallback placeholder SEO content for ${page.url}`);
    ...
    return new Response(
      JSON.stringify({
        ...
        model_used: modelUsed || 'placeholder'
      }),
      {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      }
