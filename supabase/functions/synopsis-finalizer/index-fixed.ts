...
async function generateBrandDocument(domain: string, guid: string): Promise<void> {
  try {
    ...
    // Generate brand document using GPT-4 (exact prompt from Python version)
    ...
    const brandDocumentMd = await askGPT4(brandDocumentPrompt)
...
/**
 * Ask GPT-4 (same configuration as other functions)
 */
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a helpful assistant that generates well-formatted markdown documents.'
          },
          {
            role: 'user',
            content: prompt
