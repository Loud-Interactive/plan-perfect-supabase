// process-direct-html
// Process HTML content directly for keyword analysis without scraping
...
serve(async (req) => {
  ...
  try {
    // Parse request body
    const { html, targetKeyword, url } = await req.json();
...
async function analyzeContent(
  ...
  $('h1, h2, h3, h4, h5, h6').each((_, elem) => {
    const headingText = cleanText($(elem).text().trim());
    if (headingText) {
