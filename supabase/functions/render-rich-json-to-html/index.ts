 ...
 */
function generateCallout(callout: Callout, side: 'left' | 'right'): string {
 ...
 */
function generateBodyContent(article: RichArticleJson): string {
  let html = ''
  ...
  ${section.heading}
    ...
    section.subsections.forEach((subsection, subIdx) => {
      ...
      const sentences = subsection.content.match(/[^.!?]+[.!?]+/g) || [subsection.content]
 ...
 */
function generateKeyTakeaways(keyPoints: string[]): string {
 ...
 */
function injectCSS(template: string, domain: string, pairsData: PairsData | null, customStyles: string = ''): string {
 ...
 */
function generateJsonLD(article: RichArticleJson, domain: string, heroImageUrl: string | null): string {
  const jsonLD = {
 ...
 */
function generateSocialLinks(pairsData: PairsData | null): string {
  if (!pairsData) return ''
 ...
 */
function renderToHTML(article: RichArticleJson, template: string, domain: string, pairsData: PairsData | null, customStyles: string = '', heroImageUrl: string | null = null): string {
  ...
  const aboutCompanyText = pairsData?.synopsis || 'Learn more about our company and what we do.'
  ...
  let html = template
    ...
    .replace(/{{META_DESCRIPTION}}/g, article.summary.content.substring(0, 160))
    ...
    .replace(/{{LEAD_IMAGE_URL}}/g, leadImageUrl)
    .replace(/{{LEAD_IMAGE_ALT}}/g, article.title)
    .replace(/{{ABOUT_COMPANY_TEXT}}/g, aboutCompanyText)
    .replace(/{{SOCIAL_LINKS}}/g, socialLinks)
    .replace(/{{TOC_SECTION}}/g, toc)
    .replace(/{{SUMMARY_SECTION}}/g, summary)
    .replace(/{{BODY_CONTENT}}/g, bodyContent)
    .replace(/{{KEY_TAKEAWAYS}}/g, keyTakeaways)
...
serve(async (req) => {
  ...
  try {
    ...
    let articleJson: RichArticleJson
    ...
    if (rich_json) {
      ...
      let query = supabaseClient
        .from('tasks')
        .select('client_domain, hero_image_url')
    ...
    let pairsData: PairsData | null = null
    if (clientDomain) {
      try {
        const pairsUrl = `https://pp-api.replit.app/pairs/${clientDomain}`
        console.log(`Fetching pairs data from: ${pairsUrl}`)
        ...
        if (pairsResponse.ok) {
          ...
          console.log(`Failed to fetch pairs data: ${pairsResponse.status}`)
        }
      } catch (error) {
