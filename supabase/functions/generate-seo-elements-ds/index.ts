...
async function fetchPairsDataForDomain(supabaseClient, domain) {
  const result = {};
  if (!domain) {
...
serve(async (req)=>{
  ...
  try {
    ...
    if (pageId) {
      ...
      // Fetch pairs data as soon as domain is known so it can be used early
      if (domain) {
      ...
      try {
        ...
        // Fetch pairs and domain-specific regex for keyword filtering
        if (domain) {
          pairsData = await fetchPairsDataForDomain(supabaseClient, domain);
          domainRegex = await loadDomainRegex(supabaseClient, domain);
        }
    ...
    if (!domain && pageData?.url) {
      try {
        ...
        // Ensure pairsData is available if we just derived domain late
        pairsData = await fetchPairsDataForDomain(supabaseClient, domain);
      } catch (e) {
        console.warn(`Could not extract domain from URL: ${pageData?.url}`);
      }
    }
    // If domain is still unknown, warn (pairsData will remain empty)
    if (!domain) {
    ...
    const fullPrompt = `VERY IMPORTANT: 
