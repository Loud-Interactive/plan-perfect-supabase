// PagePerfect: generate-seo-elements-gptoss (DeepSeek R1 version)
...
serve(async (req)=>{
  ...
  try {
    ...
    // Parse request body
    const { url, pageId, deepseekApiKey, modelName = 'deepseek-reasoner' } = await req.json();
    if (!url && !pageId) {
...
<paragraph>Your paragraph here</paragraph>
        ...
        if (existingRecords && existingRecords.length > 0) {
          ...
          const { error: updateError } = await supabaseClient.from('page_seo_recommendations').update({
            title,
          ...
          const { error: insertError } = await supabaseClient.from('page_seo_recommendations').insert({
            page_id: pageId,
            url: pageUrl,
            title,
            meta_description: metaDescription,
