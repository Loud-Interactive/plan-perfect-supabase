  ...
  try {
    ...
    if (jobId) {
      ...
      if (jobError) {
        ...
        // Check for existing in-progress jobs to prevent duplicates
        const { data: existingTracking, error: checkError } = await supabase
          .from('seo_processing_tracking')
          .select('id, processing_start, success')
          .eq('job_id', jobId)
          .is('success', null)
          .single();

        if (!checkError && existingTracking) {
          const startTime = new Date(existingTracking.processing_start);
          const now = new Date();
          const minutesElapsed = (now.getTime() - startTime.getTime()) / (1000 * 60);
          
          // If job started less than 10 minutes ago, skip it (already being processed)
          if (minutesElapsed < 10) {
            console.log(`Job ${jobId} is already being processed (started ${minutesElapsed.toFixed(1)} minutes ago)`);
            return new Response(
              JSON.stringify({
                success: false,
            ...
            // Job is stale, delete it and continue
            console.log(`Cleaning up stale job ${jobId} (started ${minutesElapsed.toFixed(1)} minutes ago)`);
            await supabase
              .from('seo_processing_tracking')
              .delete()
    ...
    // CRITICAL: Check if we already have complete SEO recommendations for this page (title AND h1)
    console.log(`Checking for existing SEO recommendations for page ${page.id}`);
    const { data: existingSeo, error: seoCheckError } = await supabase
      .from('page_seo_recommendations')
      .select('id, title, h1, created_at')
      .eq('page_id', page.id)
      .single();
      
    if (!seoCheckError && existingSeo && existingSeo.title && existingSeo.h1) {
      console.log(`Page ${page.id} already has complete SEO recommendations (ID: ${existingSeo.id}), skipping processing`);
      ...
      if (trackingId) {
        await supabase
          ...
          .update({
            ...
            success: true,
            error_message: 'Page already has complete SEO recommendations',
            seo_recommendation_id: existingSeo.id,
      ...
      return new Response(
        JSON.stringify({
          success: true,
          message: `Page ${page.url} already has complete SEO recommendations, skipping`,
          page: {
            id: page.id,
            url: page.url
          },
