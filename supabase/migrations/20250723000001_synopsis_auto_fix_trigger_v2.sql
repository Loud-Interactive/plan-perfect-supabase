...
DO $$
BEGIN
    -- Check if pages_completed is already a valid status
    IF NOT EXISTS (
        SELECT 1 
        FROM pg_constraint 
        WHERE conname = 'synopsis_jobs_status_check'
        AND conrelid = 'synopsis_jobs'::regclass
    ) THEN
    ...
    ELSE
        -- Drop and recreate to ensure pages_completed is included
        ALTER TABLE synopsis_jobs DROP CONSTRAINT synopsis_jobs_status_check;
        ALTER TABLE synopsis_jobs 
        ADD CONSTRAINT synopsis_jobs_status_check 
        CHECK (status IN ('pending', 'processing', 'pages_completed', 'analyzing', 'completed', 'failed', 'partially_completed'));
    END IF;
