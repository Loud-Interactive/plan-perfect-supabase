-- Synopsis Perfect Robustness Enhancements (Fixed v2)
...
-- Function to get job quality metrics (FIXED - no FILTER clause)
...
BEGIN
  ...
  -- Get page counts using standard COUNT with WHERE
  SELECT 
    COUNT(CASE WHEN status = 'completed' THEN 1 END)::INTEGER,
    COUNT(*)::INTEGER
  INTO v_pages_completed, v_pages_total
