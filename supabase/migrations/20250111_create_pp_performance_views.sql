-- PagePerfect 2.0 Performance Monitoring Views
...
SELECT 
  ...
  jsonb_agg(DISTINCT pp_batch_id) as batch_ids
FROM seo_processing_tracking
WHERE pp_batch_id IS NOT NULL
AND success = false
...
GRANT SELECT ON pp_batch_performance TO authenticated;
...
COMMENT ON VIEW pp_batch_performance IS 'Comprehensive performance metrics for each batch including timing, success rates, and domain breakdown';
